...
SELECT 
    SUM(CASE
        ...
        ELSE 0
    END) AS nulls_in_ID_nulls,
    SUM(CASE
        WHEN title IS NULL THEN 1
        ELSE 0
    END) AS nulls_in_title_nulls,
    SUM(CASE
        WHEN year IS NULL THEN 1
        ELSE 0
    END) AS nulls_in_year_nulls,
    SUM(CASE
        WHEN date_published IS NULL THEN 1
        ELSE 0
    END) AS nulls_in_date_published_nulls,
    SUM(CASE
        WHEN duration IS NULL THEN 1
        ELSE 0
    END) AS nulls_in_duration_nulls,
    SUM(CASE
        WHEN country IS NULL THEN 1
        ELSE 0
    END) AS nulls_in_country_nulls,
    SUM(CASE
        WHEN worlwide_gross_income IS NULL THEN 1
        ELSE 0
    END) AS nulls_in_worlwide_gross_income_nulls,
    SUM(CASE
        WHEN languages IS NULL THEN 1
        ELSE 0
    END) AS nulls_in_languages_nulls,
    SUM(CASE
        WHEN production_company IS NULL THEN 1
        ELSE 0
    END) AS nulls_in_production_company
FROM
    movie;
...
(
         ...
         FROM     genre           AS g
         GROUP BY genre
         ORDER BY movie_count DESC limit 3), movie_ranking AS
(
           SELECT     g.genre,
                      year,
                      title                                                                                                                                  AS movie_name ,
                      CONVERT(replace(replace(trim(worlwide_gross_income),'$',""),'INR',""),unsigned int)                                                    AS worlwide_gross_income,
                      row_number() OVER(partition BY year ORDER BY 
                            CONVERT(replace(replace(trim(worlwide_gross_income),'$',""),'INR',""),unsigned int) DESC) 
                               AS movie_rank
           FROM       movie                                                                                                                                  AS m
           INNER JOIN genre                                                                                                                                  AS g
           ON         m.id = g.movie_id,
